...
class CacheManager:
    ...
    def add_task(self, task_data: Dict) -> Optional[str]:
        """
        添加任务记录
        ...
        Args:
            task_data: 任务数据，必须包含以下字段：
                - topic: 任务主题
                - status: 任务状态 (success/error/running/draft)
                - progress: 进度百分比
                - message: 状态消息
                可选字段：
                - title: 文章标题
                ...
                - subject: 科目
                - publish_time: 发布时间
                - publish_status: 发布状态描述
                - content_type: 内容类型
                - brief: 关联的研究 brief 快照

        Returns:
            生成的 task_id (uuid)；失败返回 None
        """
        try:
            import uuid
            task_id = f"draft-{uuid.uuid4().hex[:12]}"
            ...
            optional_fields = [
                "title", "content", "tags", "images", "image_urls",
                "fact_lines", "fact_citations",
                "subject", "publish_time", "publish_status",
                "content_type", "brief", "run_id",
            ]
    ...
    def list_by_status(self, status: str, limit: int = 100) -> List[Dict]:
        ...
        try:
            cache = self._read_cache()
            return [t for t in cache if t.get("status") == status][:limit]
        except Exception as e:
            print(f"按状态查询失败: {e}")
            return []
    ...
    def update_task_status(self, task_id: str, new_status: str,
        ...
        """
        快捷修改 status (常用于 draft -> success)
        extra: 同时合并的额外字段 (例如 publish_time)
        """
